...
import torch
from scene import Scene_exr_color
import os
from tqdm import tqdm
from os import makedirs
from gaussian_renderer import render_color, render_lighting
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from gaussian_renderer import GaussianModel_color
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        # render_pkg = render_color(view, gaussians, pipeline, background, debug=True)
        render_pkg = render_color(view, gaussians, pipeline, background)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        gaussians = GaussianModel_color(dataset.sh_degree, dataset.brdf_dim, pipeline.brdf_mode, dataset.brdf_envmap_res)
        gaussians.brdf = True
        scene = Scene_exr_color(dataset, gaussians, load_iteration=iteration, shuffle=False)

        bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        if pipeline.brdf:
             brdf_mlp_path = os.path.join(dataset.model_path, f"brdf_mlp/iteration_{iteration}/brdf_mlp.hdr")
             gaussians.brdf_mlp = load_env(brdf_mlp_path, scale=1.0)

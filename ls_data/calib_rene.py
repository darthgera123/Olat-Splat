...
if __name__ == '__main__':
    ...
    if args.create_spiral:
        ...
        transforms_train,transforms_pcd,center = central_point(transforms,transform_matrix_pcl)
        ...
        transforms_val = dict()
        transforms_val["aabb_scale"] = 1.0
        frames = []
        for ind in test_indices:
            frames.append(transforms['frames'][ind])
        transforms_val["frames"] = frames
        transforms_val = update_center(transforms_val,center)
        save_json(transforms_val,os.path.join(args.output,'transforms_val.json'))
        

        transforms_test = dict()
        transforms_test["aabb_scale"] = 1.0
        frames = []

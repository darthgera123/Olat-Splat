...
class Scene_exr_color:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel_exr, load_iteration=None, shuffle=True, resolution_scales=[1.0],load_init=None):
        ...
        if self.loaded_iter:
            ...
            if self.gaussians.brdf:
                brdf_mlp_path = args.env_map_path
                self.gaussians.brdf_mlp = load_env(brdf_mlp_path, scale=1.0)
                print(f"Load envmap from: {brdf_mlp_path}")
            self.gaussians.freeze_positions()
            print("Load Initialized point cloud")
        else:
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent)
    ...
    def save(self, iteration):
        ...
        if self.gaussians.brdf:
            ...
            os.makedirs(os.path.dirname(brdf_mlp_path),exist_ok=True)
